%AIM: To find the maximum and minimum function (extreme values)
%for given function y=f(x) using MATLAB.
clear all
clc
syms x
...
a=L(1); b=L(2);
Area=int(f-g,x,a,b);
disp(['Area bounded by the curves f(x) ;nd g(x) is: ',char(Area)]);
